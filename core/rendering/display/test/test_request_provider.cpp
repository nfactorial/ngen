...
TEST( RequestProvider, Construction ) {
    ...
    EXPECT_EQ( 0, provider.getCapacity() );
    EXPECT_EQ( 0, provider.getAllocations() );
}
...
TEST( RequestProvider, Initialize ) {
    ...
    EXPECT_FALSE( provider.initialize( 0 ) );
    EXPECT_TRUE( provider.initialize( kTestAllocations ) );
    EXPECT_EQ( kTestAllocations, provider.getCapacity() );
    EXPECT_EQ( 0, provider.getAllocations() );
}
...
TEST( Requestprovider, Allocate ) {
    ...
    EXPECT_EQ( nullptr, provider.createMaterialRequest( nullptr ) );

    for ( size_t loop = 0; loop < kTestAllocations; ++loop ) {
        EXPECT_NE( nullptr, provider.createMaterialRequest( nullptr ) );    // TODO: Need to provide a real material
    }

    EXPECT_EQ( kTestAllocations, provider.getAllocations() );
    EXPECT_EQ( kTestAllocations, provider.getCapacity() );

    // Next allocation should fail, as we've used up all allocations
    EXPECT_EQ( nullptr, provider.createMaterialRequest( nullptr ) );        // TODO: Need to provide a real material
